...
public ProgramStepPanel(int lineNumber, JTextPane textDisplay, ArrayList<String> dataLines) {
		...
		JComboBox<String> stepType = new JComboBox<String>(new String[] {" ", "Open Relays", "Close Relays" , "Set Voltage", "Set Current","Wait", "Measure Voltage", "Measure Current", "Math" , "Datalog" , "Turn Off Resources"});
...
private class stepTypeListener implements ActionListener {
	...
	//			testCode.setText("Set Voltage");
			...
			case "Set Current":
				...
				break;
			case "Wait":
				JComboBox<String> Wait = new JComboBox<String>(new String[] {"10 us", "100 us", "1000 us" , "10000 us"});
				waitArray.add(Wait);
...
private class setVoltageListener implements ActionListener {
	...
	public void actionPerformed (ActionEvent e) {
		...
		setVoltage = setVoltage + " at " + ((JTextField) sVAI.next()).getText();
		setVoltage = setVoltage + " " + ((JTextField) sVAI.next()).getText() + " V,";
		setVoltage = setVoltage + " V Range " + ((JComboBox) sVAI.next()).getSelectedItem() + ",";
		setVoltage = setVoltage + " I Range " + ((JComboBox) sVAI.next()).getSelectedItem();
...
private class setCurrentListener implements ActionListener {
	...
	public void actionPerformed (ActionEvent e) {
		...
		setCurrent = setCurrent + " at " + ((JTextField) sCAI.next()).getText();
		setCurrent = setCurrent + " " + ((JTextField) sCAI.next()).getText() + " mA,";
		setCurrent = setCurrent + " V Range " + ((JComboBox) sCAI.next()).getSelectedItem() + ",";
		setCurrent = setCurrent + " I Range " + ((JComboBox) sCAI.next()).getSelectedItem();
